...
public class DownloadShareFile extends JFrame
{
	JButton b1,b2;
	JLabel l1;
	JComboBox c1;
...
public DownloadShareFile(String usr){
	super("Download Share File Screen");
	user = usr;
	p1 = new CustomPanel();
	p1.setTitle("   Download Share File Screen");
	p1.setLayout(null);
	...
	b2 = new JButton("Download(invalid key)");
	b2.setFont(f1);
	p2.add(b2);
...
public void getFiles(){
	try{
		...
		URL url = new URL("https://keyaggregatecryptoapp.appspot.com/AccessKeys");
...
public void download(){
	try{
		...
		FileInputStream fin = new FileInputStream("D:/cloud/"+owner+"/"+file);
...
public void downloadinvalid(){
	try{
		...
		fin.read(b,0,b.length);
		fin.close();
		byte decdata[] = Security.decryption(b,"abcdefghijklmnoa");
		FileOutputStream fout = new FileOutputStream("C:/"+file);
		fout.write(decdata,0,decdata.length);
		fout.close(); 
		JOptionPane.showMessageDialog(this,"File downloaded in C dir with name "+file);
